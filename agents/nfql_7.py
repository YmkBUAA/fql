...
class NFQL7Agent(flax.struct.PyTreeNode):
    """Noised Flow Q-learning v7 (NFQL7) — NFQL6 + multi-time actor weighting.
    ...
    NFQL7 changes the actor BC flow loss from one sampled flow time per
    data action to K sampled flow times per data action. Each (x_t, t)
    point gets its own Q_n residual, ESS-targeted weight, trust value, and
    per-time loss contribution.
    ...
    # -> pure BC flow loss. online (online=True) -> K-way per-time weighting.
    # ------------------------------------------------------------------
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            # ---- K-way ESS-targeted actor weighting ----
