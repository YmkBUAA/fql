...
class FQLPIAgent(flax.struct.PyTreeNode):
    """FQL with Policy-anchored Reweighted BC (ablation baseline for FQL-AR).

    Identical to FQL-AR except for how the BC-flow-loss reweighting baseline
    a' is constructed:

        FQL-AR:    a' = ODE_denoise(actor_bc_flow ; (1-t) eps + t a, t -> 1)
        FQL-PI:    a' = actor_onestep_flow(s, eps)             (this file)

    That is, a' is taken from the *one-step distilled actor's* output instead
    of the multi-step BC flow's partial denoise of the data action a. The
    rest of the pipeline (ESS-targeted softmax, MAD normalization, critic R^2
    gate, distillation, Q loss) is byte-for-byte identical to FQL-AR.

    Why this baseline matters as an ablation:

    - a' = pi(s) does not depend on the data action a -> Cov[Q(a), Q(a')] = 0
      over the per-state distribution of a. The control-variate variance
      reduction that FQL-AR enjoys (Cov > 0 from the Lipschitz ODE) is gone.
    - pi is trained to argmax_a Q(s, a), so Q(s, pi(s)) systematically tracks
      the high end of Q(s, .). Delta = Q(a) - Q(pi(s)) is biased toward
      negative values, which reweights *down* almost every data sample
      uniformly rather than separating good ones from bad ones.
    - pi may drift OOD as q_loss pushes it greedily; Q(s, pi(s)) is then
      queried on points the BC flow would not produce, reintroducing the
      offline RL bootstrap loop that FQL-AR's flow-anchored a' avoids by
      construction.
    - a' is single-mode (deterministic head, fresh noise per sample), so on
      multi-modal data, all data actions in the non-greedy mode get
      systematically downweighted -> mode collapse pressure.
    - There is no continuous locality knob t. The "perturbation depth" of
      a' is fixed by whatever the distilled head currently does.

    The expected outcome of this ablation is that fql_pi underperforms both
    FQL-AR and FQL-V (and possibly plain FQL) on multi-modal data sets,
    confirming that FQL-AR's gains come from the *flow-anchored* baseline
    construction rather than from the ESS+MAD reweighting machinery alone.
    """
    ...
    def actor_loss(self, batch, grad_params, rng, online):
        """Policy-anchored reweighted BC actor loss = Delta-weighted BC flow + distill + Q."""
        batch_size, action_dim = batch['actions'].shape
        a = batch['actions']

        # ----- Branch A: advantage via the one-step distilled policy ------
        # a' is the deterministic head's output for fresh noise. It depends
        # only on s (and the noise draw), NOT on the data action a. This is
        # the key contrast with FQL-AR's flow-anchored a' which is sample
        # -anchored via partial denoising of a.
        rng, eps_rng = jax.random.split(rng)
        eps_adv = jax.random.normal(eps_rng, (batch_size, action_dim))
        a_prime = self.network.select('actor_onestep_flow')(
            batch['observations'], eps_adv
        )
        a_prime = jnp.clip(a_prime, -1, 1)
        ...
        return actor_loss, {
            ...
            'q': q.mean(),
            'mse': mse,
            'pibase/delta_mean': delta.mean(),
            'pibase/delta_std': delta.std(),
            'pibase/dist_a_aprime_mean': dist_a_aprime.mean(),
            'pibase/dist_a_aprime_p50': jnp.median(dist_a_aprime),
            'pibase/frac_a_better': frac_a_better,
            'pibase/q_a_mean': q_a.mean(),
            'pibase/q_aprime_mean': q_aprime.mean(),
            'beta_mad': beta_mad,
            'tau_star': tau_star,
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            # ---- BC flow CFM samples per state ----
            n_actor_time_samples=1,
            # ---- ESS-targeted reweighting (identical to FQL-AR) ----
            ess_target=0.7,
            weighted_bc_online_only=True,
            # ---- Critic R^2 reliability gate (identical to FQL-AR) ----
